...
func (s *KeeperTestSuite) TestMsgServer_EditDataProxy() {
	...
	for _, tt := range tests {
		s.Run(tt.name, func() {
			...
			if proxyConfig.FeeUpdate != nil {
				updateScheduled, err := s.keeper.FeeUpdates.Has(s.ctx, collections.Join(proxyConfig.FeeUpdate.UpdateHeight, pubKeyBytes))
				s.Require().NoError(err)
	...
	s.Run("Updating the fee for a proxy that already has a pending update should cancel the old update", func() {
		...
		firstUpdateScheduled, err := s.keeper.FeeUpdates.Has(s.ctx, collections.Join(firstUpdateHeight, pubKeyBytes))
		s.Require().NoError(err)
		...
		secondUpdateScheduled, err := s.keeper.FeeUpdates.Has(s.ctx, collections.Join(secondUpdateHeight, pubKeyBytes))
		s.Require().NoError(err)
		s.Require().True(secondUpdateScheduled)

		firstUpdateNoLongerScheduled, err := s.keeper.FeeUpdates.Has(s.ctx, collections.Join(firstUpdateHeight, pubKeyBytes))
		s.Require().NoError(err)
